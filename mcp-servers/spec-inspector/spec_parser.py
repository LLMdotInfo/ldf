...
class SpecParser:
    ...
    def parse_tasks(self, spec_name: str) -> list[dict[str, Any]]:
        ...
        **Estimated:** 2 hours
        ...
        # Pattern 1: Header format (## Task X.Y or ### Task X.Y)
        task_pattern = r"#{2,3}\s+(?:Task\s+|T-)([\d.]+):\s+(.+?)(?=\n#{2,3}\s|\Z)"
        task_matches = list(re.finditer(task_pattern, content, re.DOTALL | re.MULTILINE))
        ...
        for match in task_matches:
            ...
            # Detect status from title markers
            title_lower = task_title.lower()
            if "\u2705" in task_title or "complete" in title_lower or "implemented" in title_lower:
                detected_status = "completed"
            elif "\U0001f504" in task_title or "in progress" in title_lower:
                detected_status = "in_progress"
            elif "\U0001f6ab" in task_title or "blocked" in title_lower or "(future)" in title_lower:
                detected_status = "blocked"
            else:
                detected_status = None

            # Extract metadata
            estimated_match = re.search(r"\*\*Estimated:\*\*\s*([\d.]+)\s*hours?", task_content, re.IGNORECASE)
            status_match = re.search(r"\*\*Status:\*\*\s*(.+?)(?=\n|$)", task_content, re.IGNORECASE)
            deps_match = re.search(r"\*\*Dependencies:\*\*\s*(.+?)(?=\n|$)", task_content)

            # Extract checklist items
            checklist_items = re.findall(r"- \[([ x])\] (.+)", task_content)
            checklist_total = len(checklist_items)
            checklist_completed = len([item for item in checklist_items if item[0] == "x"])
            ...
            if file_complete:
                ...
                if "\u2705" in status_value:
                    ...
                    final_status = normalize_status(status_value)
            elif detected_status:
        ...
        # Pattern 2: Bullet format - only parse if no header-format tasks found
        if not tasks:
            tasks = self._parse_bullet_tasks(content)
    ...
    def _parse_bullet_tasks(self, content: str) -> list[dict[str, Any]]:
        """Parse bullet-format tasks."""
        tasks = []

        bullet_pattern = r"^- \[([x ])\] \*\*Task\s+([\d.]+):\*\*\s+(.+?)(?=\n- \[|\n\n## |\n---|\Z)"
        ...
        for match in bullet_matches:
            ...
            sub_items = re.findall(r"^\s+- \[([x ])\]", task_content, re.MULTILINE)
            checklist_total = len(sub_items)
            checklist_completed = len([s for s in sub_items if s == "x"])
    ...
    def parse_answerpack(self, spec_name: str, answerpack_name: str) -> dict[str, Any]:
        ...
        if not answerpack_file.exists():
            # Check central
            answerpack_file = self.project_root / ".ldf" / "answerpacks" / spec_name / answerpack_name
