...
class TestExtractTasks:
    ...
    def test_extracts_tasks_with_numbered_format(self):
        """Test extraction of tasks with numbered format."""
        content = """# Tasks
...
"""
    ...
    def test_handles_task_without_subtasks(self):
        """Test handling of task headers without subtasks."""
        content = """## Phase 1
...
"""
        ...
        # Should only find the two actual task headers, not the inline references
        assert len(tasks) == 2
        assert tasks[0].id == "1.1"
...
"""
    ...
    def test_ignores_task_ids_without_heading_markers(self):
        """Test that task IDs without # markers are not matched."""
        content = """## Tasks

Some paragraph mentioning 1.1: this should not match.
Another line with 2.3 that should not match.
...
"""
    ...
    def test_requires_heading_marker(self):
        """Test that heading marker is required for task detection."""
        content = """
Task 1.1: This should not match (no heading marker)
1.1: This should not match (no heading marker)

### Task 1.1: This should match
- [ ] Step one
"""
        tasks = extract_tasks(content)

        # Only the one with heading marker should match
        assert len(tasks) == 1
        assert tasks[0].id == "1.1"
...
"""
    ...
    def test_supports_both_heading_and_checklist_formats(self):
        """Test that both heading and checklist formats work in same file."""
        content = """# Tasks

### Task 1.1: Heading format task
- [ ] Step one

- [ ] **Task 1.2:** Checklist format task
  - [ ] Step one

## Phase 2

### Task 2.1: Another heading task
- [ ] Implementation

- [x] **Task 2.2:** Another checklist task
  - [x] Done
"""
        tasks = extract_tasks(content)
        ...
        assert len(tasks) == 4
        assert tasks[0].id == "1.1"
        assert tasks[0].title == "Heading format task"
        assert tasks[1].id == "1.2"
        assert tasks[1].title == "Checklist format task"
        assert tasks[2].id == "2.1"
        assert tasks[2].title == "Another heading task"
        assert tasks[3].id == "2.2"
        assert tasks[3].title == "Another checklist task"
...
"""
    ...
    def test_checklist_format_without_bold_markers(self):
        """Test extraction of checklist tasks without bold markers."""
        content = """# Tasks
...
- [x] Task 2.1: Configure environment
  - [x] Install packages
"""
        tasks = extract_tasks(content)
        ...
        assert tasks[0].status == "pending"
        assert tasks[1].id == "2.1"
        assert tasks[1].title == "Configure environment"
...
### Task 2.1: Heading format task

### Task 2.1.1: Subtask in heading format
"""
        tasks = extract_tasks(content)
        ...
        assert tasks[3].title == "Heading format task"
        assert tasks[4].id == "2.1.1"
        assert tasks[4].title == "Subtask in heading format"
