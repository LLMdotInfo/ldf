...
class TestRunMcpHealth:
    ...
    def test_runs_checks_for_configured_servers(self, temp_project: Path):
        ...
        assert report.servers[0].name == "spec-inspector"
    ...
    def test_uses_defaults_without_config(self, temp_project: Path):
        ...
        assert "spec-inspector" in server_names
        assert "coverage-reporter" in server_names
...
class TestPrintHealthReport:
    ...
    def test_prints_report(self, capsys):
        ...
        report = HealthReport(
            servers=[
                ServerHealth("spec-inspector", HealthStatus.READY, "5 specs"),
                ServerHealth("coverage-reporter", HealthStatus.WARNING, "No file"),
                ServerHealth("db-inspector", HealthStatus.SKIP, "Not configured"),
            ]
        )
        ...
        assert "spec-inspector" in captured.out
