...
def check_spec_inspector(project_root: Path) -> ServerHealth:
    """Check spec-inspector server health.
...
def check_coverage_reporter(project_root: Path) -> ServerHealth:
    """Check coverage-reporter server health.
...
def check_db_inspector(project_root: Path) -> ServerHealth:
    """Check db-inspector server health.
...
def run_mcp_health(project_root: Path | None = None) -> HealthReport:
    ...
    if not configured_servers:
        configured_servers = ["spec-inspector", "coverage-reporter"]

    # Run health checks
    report = HealthReport(servers=[])

    server_checks = {
        "spec-inspector": check_spec_inspector,
        "coverage-reporter": check_coverage_reporter,
        "db-inspector": check_db_inspector,
    }
...
def print_health_report(report: HealthReport) -> None:
    ...
    console.print("━" * 50)
    console.print(
        f"[bold]{report.ready_count}[/bold] ready, "
        f"[bold]{report.skipped_count}[/bold] skipped"
    )
