    ...
    try:
        ...
        if name == "list_tables":
            result = await query.list_tables(
                schema=schema,
                include_rls=arguments.get("include_rls_status", False)
            )
        elif name == "get_table_schema":
            result = await query.get_table_schema(
                table_name=arguments["table_name"],
                schema=schema
            )
        elif name == "get_rls_policies":
            result = await query.get_rls_policies(
                table_name=arguments["table_name"],
                schema=schema
            )
            ...
            result = await query.get_indexes(
                table_name=arguments["table_name"],
                schema=schema
            )
