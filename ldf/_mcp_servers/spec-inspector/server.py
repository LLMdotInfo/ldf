...
logger = logging.getLogger("spec-inspector")
...
async def validate_answerpacks(spec_name: str) -> dict[str, Any]:
    ...
    for answerpack in answerpacks:
        ...
        template_markers = [
            "REPLACE_ME", "TODO:", "PLACEHOLDER", "YOUR_",
            'feature_name: ""', "feature_name: ''"
        ]
        for marker in template_markers:
            if marker in content:
                issues.append({
                    "file": answerpack.name,
                    "issue": f"Contains template marker: {marker}"
                })
                break
    ...
    try:
        async with semaphore:
            ...
            process = await asyncio.create_subprocess_exec(
                sys.executable, "-m", "ldf.cli", "lint", spec_name,
