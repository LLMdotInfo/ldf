...
def load_active_guardrails(project_root: Path) -> list[dict[str, Any]]:
    """Load active guardrails from .ldf/guardrails.yaml and framework."""
    guardrails = []
    ...
    project_guardrails = project_root / ".ldf" / "guardrails.yaml"
    if project_guardrails.exists():
        with open(project_guardrails) as f:
        ...
        guardrails = [g for g in guardrails if g["id"] not in disabled and g["name"] not in disabled]

    # If no guardrails loaded, use defaults
    if not guardrails:
...
class GuardrailTracker:
    ...
    def get_coverage_matrix(self, spec_name: str) -> dict[str, Any]:
        ...
        for guardrail in self.guardrails:
            if guardrail["id"] not in matrix_ids:
                ...
                name_found = any(
                    guardrail["name"].lower() in row["guardrail"].lower()
                    for row in matrix
                )
                if not name_found:
    ...
    def get_guardrail_tasks(self, spec_name: str, guardrail_id: int) -> list[dict[str, Any]]:
        ...
        for task in tasks:
            ...
            # Find task section
            task_section_pattern = rf"#{{2,3}}\s+(?:Task\s+|T-){re.escape(task_id)}:(.+?)(?=\n#{{2,3}}\s|\Z)"
            task_match = re.search(task_section_pattern, content, re.DOTALL)

            if not task_match:
                task_section_pattern = rf"^- \[[x ]\] \*\*Task\s+{re.escape(task_id)}:\*\*(.+?)(?=\n- \[|\n\n## |\Z)"
    ...
    def compare_specs(self, spec1: str, spec2: str) -> dict[str, Any]:
        ...
        return {
            ...
            "coverage1": coverage1,
            "coverage2": coverage2,
            "compliance_diff": coverage2.get("compliance_rate", 0) - coverage1.get("compliance_rate", 0)
        }
