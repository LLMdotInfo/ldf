...
def check_for_updates(project_root: Path) -> UpdateInfo:
    ...
    current_version = config.get("framework_version", "0.0.0")
...
def get_update_diff(
    project_root: Path, components: list[str] | None = None
) -> UpdateDiff:
...
def _diff_question_packs(project_root: Path, config: dict, diff: UpdateDiff) -> None:
    ...
    for pack_name in configured_packs:
        ...
        dest_file = dest_dir / f"{pack_name}.yaml"
