...
def check_mcp_servers(project_root: Path) -> CheckResult:
    ...
    try:
        ...
        for server in configured_servers:
            # Server directories use hyphens (spec-inspector, coverage-reporter)
            server_dir = server.replace("_", "-")
            try:
                # Check if server.py exists in the server directory
                server_path = mcp_servers_pkg.joinpath(server_dir, "server.py")
...
def check_mcp_deps(project_root: Path) -> CheckResult:
    ...
    try:
        ...
        # Check coverage package (needed by coverage-reporter)
        if "coverage-reporter" in config.get("mcp_servers", []):
            try:
...
def check_mcp_json(project_root: Path) -> CheckResult:
    """Check if .claude/mcp.json is up to date with config."""
    mcp_json_path = project_root / ".claude" / "mcp.json"
    config_path = project_root / ".ldf" / "config.yaml"
    ...
    if not mcp_json_path.exists():
        return CheckResult(
            ...
            status=CheckStatus.PASS,
            message=".claude/mcp.json not present (optional)",
        )
    ...
    if not config_path.exists():
        return CheckResult(
            name="MCP config file",
            status=CheckStatus.WARN,
            message=".claude/mcp.json exists but no LDF config",
        )
    ...
    try:
        ...
        if not configured_servers:
            return CheckResult(
                name="MCP config file",
                status=CheckStatus.PASS,
                message=".claude/mcp.json exists (no LDF servers configured)",
            )
        ...
        for name in mcp_servers:
            if "ldf._mcp_servers" in str(mcp_servers[name].get("args", [])):
                # Normalize back to hyphenated form (spec_inspector -> spec-inspector)
        ...
        if missing_from_json:
            return CheckResult(
                name="MCP config file",
                status=CheckStatus.WARN,
                message=f".claude/mcp.json missing: {', '.join(missing_from_json)}",
                fix_hint="Run: ldf mcp-config > .claude/mcp.json",
            )

        return CheckResult(
            name="MCP config file",
            status=CheckStatus.PASS,
            message=".claude/mcp.json up to date",
        )
