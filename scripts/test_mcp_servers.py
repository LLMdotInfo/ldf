...
Usage:
    ...
    python scripts/test_mcp_servers.py --server spec-inspector
...
def validate_spec_inspector(mcp_dir: Path, verbose: bool = False) -> list[tuple[bool, str]]:
    """Validate spec-inspector MCP server."""
    results = []
    server_dir = mcp_dir / "spec-inspector"
...
def validate_coverage_reporter(mcp_dir: Path, verbose: bool = False) -> list[tuple[bool, str]]:
    """Validate coverage-reporter MCP server."""
    results = []
    server_dir = mcp_dir / "coverage-reporter"
...
def validate_db_inspector_template(mcp_dir: Path, verbose: bool = False) -> list[tuple[bool, str]]:
    """Validate db-inspector template (optional server)."""
    results = []
    server_dir = mcp_dir / "db-inspector"
...
def main():
    ...
    parser.add_argument(
        "--server",
        choices=["spec-inspector", "coverage-reporter", "db-inspector", "all"],
        default="all",
    ...
    if args.server == "all":
        servers_to_check = ["spec-inspector", "coverage-reporter", "db-inspector", "setup"]
    else:
