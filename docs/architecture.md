...
```
ldf/_mcp_servers/
├── spec_inspector/     # Query specs, tasks, guardrail coverage
├── coverage_reporter/  # Test coverage metrics
└── db_inspector/       # Database schema inspection (template)
```

**How they integrate:**

1. `ldf mcp-config` generates `.agent/mcp.json`
...
def test_detection(tmp_path):
    # Setup
    (tmp_path / ".ldf").mkdir()
    (tmp_path / ".ldf" / "config.yaml").write_text("framework_version: '1.0.0'")
