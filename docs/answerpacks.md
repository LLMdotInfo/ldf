...
## Value Demonstration: Before vs. After

### Without Answerpacks (Traditional Approach)

**Day 1**: Developer starts implementing authentication

```python
# Developer guesses at implementation details
def login(email, password):
    user = db.get_user(email)
    # TODO: How should we hash passwords?
    # TODO: What's the session timeout?
    # TODO: Do we need MFA?
    if user.password == password:  # Oops, no hashing!
        return {"token": "abc123"}  # Oops, not a real JWT!
```

**Day 3**: Code review reveals issues
- "You need bcrypt for password hashing!"
- "JWT tokens should have expiry!"
- "What about rate limiting?"
- "We need MFA for compliance!"

**Day 4-7**: Major rewrite
- Implement proper hashing (2 days)
- Add JWT with refresh tokens (1 day)
- Add rate limiting (1 day)
- Research and implement TOTP MFA (3 days)

**Total cost**: 7 days + complete rewrite + frustrated team

---

### With Answerpacks (LDF Approach)

**Day 1**: Answer security question-pack (30 minutes)
...
answers:
  ...
  mfa:
    required: true
    method: "TOTP via authenticator app"

  rate_limiting: "5 login attempts per 15 min per IP"
```

**Day 1**: Implement correctly from the start
...
def login(email: str, password: str) -> LoginResponse:
    # All decisions already made in answerpack!
    user = db.get_user(email)
    ...
    # Answer: JWT with 15-min expiry
    token = jwt.encode({
        "user_id": user.id,
        "exp": now() + timedelta(minutes=15)
    }, secret_key)

    # Answer: TOTP MFA required
    if not pyotp.TOTP(user.mfa_secret).verify(mfa_code):
        return error_response(401, "INVALID_MFA")

    return {"access_token": token, "refresh_token": ...}
```

**Day 3**: Code review is clean
- ✅ Proper password hashing
- ✅ JWT with correct expiry
- ✅ MFA implemented
- ✅ Rate limiting in place

**Total cost**: 30 minutes upfront + correct implementation + happy team

**Savings**: 6.5 days + no rework
